...
chat.post('/registration', (req, res)=>{
            ...
            form.parse(req, (err, fields, files)=>{
                ...
                let uname = fields.uame;
                let pwd = fields.pwd;
                let img = "/userImages/"+phone + ".jpg";
                let imgLink = "C:/chat application/public/userImages/"+phone+".jpg";
                users.find({phone: phone}, (err, result)=>{
                    ...
                    if(result.length==0){
                        fs.rename(tmp, imgLink, ()=>{
                            ...
                            nUser.save().then(data=>{
                                console.log("here");
                                res.render('register', {phone:phone, state: "successful"})
                            })
                        })
                    }else{
                        console.log("there")
                        res.render('signUp', {phone: phone, status: "unsuccessful"})
                    }
                })
                //Loading Chat Page
                chat.get('/chat', (req,res)=>{
                    users.find({phone:phone},(err,resuk)=>{
                       let pic = resuk[0].picture;
                        users.find({}, (err, result)=>{
                            groups.find({}, (err, groups)=>{
                                res.render('chatpage', {user: phone,pic: pic, users: result, groups: groups})
                            })
                        })
                    })
        ...
        if(result.length==0){
            ...
            users.find({}, (err, result)=>{
                groups.find({}, (err, groups)=>{
                    ...
                    res.render('chatpage', {user: pn,pic: pic, users: result, groups: groups})
                })
            })
        }
    })
...
//Checking Login Credentials
    users.find({phone: pn, pwd: pwd}, (err, result)=>{
        ...
        if(result.length==0){
            ...
            users.find({}, (err, result2)=>{
                groups.find({}, (err, groups)=>{
                    let apiResult = {result,groups}
                        // res.send(apiResult);
                    
                  
                    res.render('chatpage', {user: pn, users: result2, groups: groups})
                })
            })
        }
    })
